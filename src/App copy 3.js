...
function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (nonEditRef.current) {
        const element = nonEditRef.current;
        const totalHeight = element.scrollHeight - element.offsetHeight; // Total scrollable height
        const currentScroll = element.scrollTop; // Current scroll position
        const scrollPercentage = (currentScroll / totalHeight) * 5; // Scroll progress as a percentage
        
        // Now, apply this percentage to move the background.
        // This assumes the background image has room to scroll. Adjust as necessary.
        if (backgroundRef.current) {
          backgroundRef.current.style.transform = `translateY(-${scrollPercentage}%)`;
        }
      }
    };
  
    const nonEditableElement = nonEditRef.current;
    nonEditableElement.addEventListener('scroll', handleScroll);
  
    // Cleanup function to remove the event listener
    return () => {
      if (nonEditableElement) {
        nonEditableElement.removeEventListener('scroll', handleScroll);
      }
    };
